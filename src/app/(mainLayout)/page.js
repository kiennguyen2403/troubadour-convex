...
export default function Home() {
        ...
        <Box sx={{
            ...
            <Grid container spacing={2}>
                {live.length > 0 ? live.map((item) =>
                    ...
                    <Typography textAlign="center" width="100%" marginTop="5%" marginBottom="5%">
                        No media found
                    </Typography>}
            </Grid>
        </Box>
    ...
    const Custom =
        <Box sx={{
            ...
            <Grid container spacing={2}>
                {custom.length > 0 ? custom.map((item) =>
                    <Grid item xs={4}>
                        <AudioButton image={item.image} title={item.title} artist={item.description} eventHandler={() => getAudio(item.url, item.title, item.artist)} />
                    </Grid>) :
                    <Typography textAlign="center" width="100%" marginTop="5%" marginBottom="5%">
                        No media found
                    </Typography>}
    ...
    return (
        <ClippedDrawer Component={isLoading ? [Loading] : [LiveStream, Custom, Recent, Top]} />
    )
}