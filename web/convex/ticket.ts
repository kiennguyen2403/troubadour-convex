...
export const post = internalMutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      return await ctx.db.insert("ticket", {
        name: args.name,
        fee: args.fee,
        status: "active",
      });
...
export const get = query({
  handler: async (ctx) => {
    try {
      return await ctx.db.query("ticket").collect();
    } catch (e) {
      console.log(e);
      return "failure";
    }
...
export const getByUser = query({
  ...
  handler: async (ctx, { userId }) => {
    try {
      if (!userId) {
        throw new Error("No user provided for ticket get by user");
      }
      return await ctx.db
        .query("ticket")
...
export const patch = internalMutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      return await ctx.db.patch(args.id, data);
    } catch (e) {
      console.log(e);
      return "failure";
    }
